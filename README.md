...
![Standby169 Preview](https://via.placeholder.com/800x450.png?text=Standby169+Dashboard)
*(Lưu ý: Bạn có thể thay thế link ảnh trên bằng ảnh chụp màn hình thực tế của trang web)*

## ✨ Tính năng chính

- **🎨 Giao diện Glassmorphism:** Thiết kế tối màu với hiệu ứng kính mờ (blur), bo góc mềm mại và các điểm nhấn ánh sáng tinh tế.
...
- **🎵 Trình phát nhạc YouTube:**
  ...
  - Selector chọn Playlist nhanh (nhạc Remix Việt hoặc Deepression Mix).
...
```javascript
// Thay đổi ID này thành ID Discord của bạn
// Lưu ý: Bạn cần tham gia server Discord của Lanyard để API hoạt động
const USER_ID = "YOUR_DISCORD_USER_ID";
2. Thay đổi Playlist nhạc
Tìm phần #playlist-selector trong HTML và thay đổi ID Playlist YouTube trong hàm goFullscreenAndStart:

HTML
<button class="glass-btn" onclick="goFullscreenAndStart('YOUR_PLAYLIST_ID_1')">Tên Playlist 1</button>
<button class="glass-btn" onclick="goFullscreenAndStart('YOUR_PLAYLIST_ID_2')">Tên Playlist 2</button>
...
```
git clone [https://github.com/Astear17/Standby169.git](https://github.com/Astear17/Standby169.git)
```
Mở file index.html trực tiếp bằng trình duyệt hoặc sử dụng Live Server của VS Code.

Made with 💜 by Astear17
